...
/// @notice Minimal placeholder adapter for Euler RLUSD vault flow
contract EulerAdapter is IStrategyAdapter, Ownable {
    using SafeERC20 for IERC20;

    address public immutable UNDERLYING; // e.g., RLUSD
    address public manager;

    event ManagerUpdated(address indexed mgr);

    modifier onlyManager() {
        require(msg.sender == manager || msg.sender == owner(), "EA: not mgr");
        _;
    }
