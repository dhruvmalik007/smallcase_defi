...
/// @notice Minimal placeholder adapter for Lido Earn (ETH/wstETH)
contract LidoEarnAdapter is IStrategyAdapter, Ownable {
    using SafeERC20 for IERC20;

    address public immutable UNDERLYING; // e.g., wstETH or WETH as configured
    address public manager;

    event ManagerUpdated(address indexed mgr);

    modifier onlyManager() {
        require(msg.sender == manager || msg.sender == owner(), "LEA: not mgr");
        _;
    }
