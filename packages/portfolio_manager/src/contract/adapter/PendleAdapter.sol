...
/// @notice Minimal placeholder adapter for Pendle sUSDCe/PT flow
contract PendleAdapter is IStrategyAdapter, Ownable {
    using SafeERC20 for IERC20;

    address public immutable UNDERLYING;
    address public manager; // router/PM

    event ManagerUpdated(address indexed mgr);

    modifier onlyManager() {
        require(msg.sender == manager || msg.sender == owner(), "PA: not mgr");
        _;
    }
